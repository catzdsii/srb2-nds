...
static UINT8 *R_GenerateTexture(size_t texnum)
{
	...
	if (texture->patchcount == 1)
	{
		...
		{
			column_t *col = (column_t *)((UINT8 *)realpatch + LONG(realpatch->columnofs[x]));
		...
		if (holey)
		{
			...
			texturecolumnofs[texnum] = (UINT32 *)(void *)(block + 8);
			blocktex = block;
			for (x = 0; x < texture->width; x++)
				texturecolumnofs[texnum][x] = LONG(LONG(texturecolumnofs[texnum][x]) + 3);
			goto done;
		}
