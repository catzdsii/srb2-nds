		...
		if (myfgets(s, 1024, f))
		{
			...
			*(tmp-1) = '\0';
			...
			else if (fastcmp(word, "FORCECHARACTER"))
			{
				...
				strlcpy(mapheaderinfo[num-1]->interscreen, word2, sizeof(mapheaderinfo[num-1]->interscreen));
		...
		if (myfgets(s, 1024, f))
		{
			...
			*(tmp-1) = '\0';
			...
			else if (fastcmp(word, "GAMEDATA"))
			{
				...
				strlcpy(timeattackfolder, gamedatafilename, sizeof(timeattackfolder));
				timeattackfolder[min(filenamelen, sizeof (timeattackfolder) - 1)] = '\0';

				strncpy(savegamename, timeattackfolder, sizeof (savegamename));
